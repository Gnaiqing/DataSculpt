...
"""
    ...
    if example_per_class > 0:
        ...
        for c in range(n_class):
            active_indices = np.nonzero(np.array(dataset.labels) == c)[0]
        ...
        for idx in example_indices:
            # Use accuracy to choose example keywords
            item = dataset.examples[idx]["text"]
            label = dataset.labels[idx]
            tokens = nltk.word_tokenize(item.lower())
            if dataset_name in relation_extraction_datasets:
                entity1 = dataset.examples[idx]["entity1"]
                entity2 = dataset.examples[idx]["entity2"]
                user_input = f"{item} <{entity1}> <{entity2}>"
            else:
                user_input = item
            ...
            for token in tokens:
                ...
                cov, acc = lf.get_cov_acc(dataset)
                if acc > 0.7:
                    candidate_keywords.append(token)
            if len(candidate_keywords) == 0:
                candidate_keywords.append("NA")

            candidate_keywords = " ".join(candidate_keywords)
            # TODO: add automatic explanation generation
            example = "User:\n{}\n Response:\n KEYWORDS: {}\n LABEL: {}\n".format(user_input, candidate_keywords, label)
            examples += example
    else:
        examples = ""
...
INTERACTION FORMAT: 
{}
{}""".format(system_role, task, task_info, class_info, interaction_format, examples)
